...
var lightFormatters = {
  ...
  S(date, token) {
    ...
    const fractionalSeconds = Math.floor(milliseconds2 * Math.pow(10, numberOfDigits - 3));
...
function throwProtectedError(token, format2, input) {
  if (token === "YYYY") {
    throw new RangeError(`Use \`yyyy\` instead of \`YYYY\` (in \`${format2}\`) for formatting years to the input \`${input}\`; see: https://github.com/date-fns/date-fns/blob/master/docs/unicodeTokens.md`);
  } else if (token === "YY") {
    throw new RangeError(`Use \`yy\` instead of \`YY\` (in \`${format2}\`) for formatting years to the input \`${input}\`; see: https://github.com/date-fns/date-fns/blob/master/docs/unicodeTokens.md`);
  } else if (token === "D") {
    throw new RangeError(`Use \`d\` instead of \`D\` (in \`${format2}\`) for formatting days of the month to the input \`${input}\`; see: https://github.com/date-fns/date-fns/blob/master/docs/unicodeTokens.md`);
  } else if (token === "DD") {
    throw new RangeError(`Use \`dd\` instead of \`DD\` (in \`${format2}\`) for formatting days of the month to the input \`${input}\`; see: https://github.com/date-fns/date-fns/blob/master/docs/unicodeTokens.md`);
  }
}
...
function formatRFC3339(date, options) {
  ...
  if (fractionDigits > 0) {
    ...
    const fractionalSeconds = Math.floor(milliseconds2 * Math.pow(10, fractionDigits - 3));
...
function interval(start, end, options) {
  ...
  if (!end)
    throw new TypeError("End date is required");
...
function isWithinInterval(date, interval2) {
  ...
  const [startTime, endTime] = [+toDate(interval2.start), +toDate(interval2.end)].sort();
